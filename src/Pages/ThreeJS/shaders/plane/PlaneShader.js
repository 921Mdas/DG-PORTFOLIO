...
uniform vec2 uElevationOpposite;
uniform float uTime;
attribute float aRandom;

// declaring new empty variables
varying vec2 vUv;
...
float cnoise(vec3 P){
 ...
 void main(){
   ...
   float elevation = sin(modelPosition.x * uElevationOpposite.x) * sin(modelPosition.z * uElevationOpposite.y) * uElevation;
...
function PlaneShader() {
  ...
  useEffect(() => {
    ...
    const vArray = new Float32Array(count);
    for (let i = 0; i < count; i++) {
      vArray[i] = Math.random();
    }
    planeRef.current.setAttribute(
      "aRandom",
      new THREE.BufferAttribute(vArray, 1)
    );
  }, []);
